...
def generate_child_node(obstacle_matrix, map_boundary_matrix, parent_node, goal_node_coord, action, map_grid, map_height, map_width, step_size):
    ...
    parent_cost_to_come = parent_node[1][5]
    parent_total_cost = parent_node[1][0]
    ...
    ##### Issue computing c2c, should be cost_of_action + parent_cost_to_come not parent_total_cost
...
def map_obstacle_freespace_matrix(map_grid, map_height, map_width):  
    ...
    map_boundary_matrix[obstacle_space] = -1 # Set obstacle space to -1 # round(-1,1)

    # Set the starting point to 0
    # map_boundary_matrix[0, 0] = -1 #round(0,1)
...
def astar_approach_alg(obstacle_matrix, map_boundary_matrix, initial_node_coord, goal_node_coord, map_grid, map_height, map_width, th_initial, th_goal, step_size):
    ...
    while (len(open_dict) != 0): # Stop search when node queue is empty 
        ...
        while i < 6: # Iterate for 8 times -> 5 moves
            ...
            # Generate child node
            child_cost_to_come, total_cost, valid_move, child_node_x_valid, child_node_y_valid, child_theta, is_node_obstacle = generate_child_node(obstacle_matrix, map_boundary_matrix, curr_node, goal_node_coord, i, map_grid, map_height, map_width, step_size)
            ...
            if valid_move == True: # Child node is valid but has not been explored yet
                ...
                if (euclidean_dist <= euclid_dist_threshold) and (abs(child_theta - th_goal) <= theta_threshold): # Child node equals goal node
                    ...
                    if (is_in_open == False): # New child, child has not been expored and is not is in open dictionary
                        ...
                        child_total_cost_stored = open_dict[(x_replace, y_replace)][0]  # WE WANT TOTAL COST

                        # If CHILDTOTAL COST < STOREDTOTAL COST
                        if total_cost < child_total_cost_stored: #and (abs(child_theta - child_theta_val_stored) <= theta_threshold): # update cost to come value and parent node
...
def main_func(): 
    ...
    start_node_color = (255, 255, 0) #(38, 25, 216)
