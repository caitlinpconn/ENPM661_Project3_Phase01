...
def generate_child_node(obstacle_matrix, map_boundary_matrix, parent_node, goal_node_coord, action, map_grid, map_height, map_width, step_size):
    ...
    # cost_to_go = abs(child_node_x - goal_node_coord[0]) + abs(child_node_y - goal_node_coord[1])

    astar_weight = 1
    total_cost = new_cost_to_come + (astar_weight*cost_to_go)
...
def goal_node_found(goal_found, visited_queue, child_node_x_valid, child_node_y_valid, total_cost, node_idx, curr_parent_idx, curr_node_coord, child_theta):
    goal_found = True
    
    node_idx = node_idx + 1 

    child_node = ((child_node_x_valid, child_node_y_valid),(total_cost, node_idx, curr_parent_idx, curr_node_coord, child_theta))
    
    visited_queue[(child_node_x_valid, child_node_y_valid)] = (total_cost, node_idx, curr_parent_idx, curr_node_coord, child_theta)
...
def astar_approach_alg(obstacle_matrix, map_boundary_matrix, initial_node_coord, goal_node_coord, map_grid, map_height, map_width, th_initial, th_goal, step_size):
    ...
    if (initial_node_coord[0] == goal_node_coord[0] and initial_node_coord[1] == goal_node_coord[1] and abs(th_initial - th_goal) <= theta_threshold):
        ...
        visited_queue, goal_found, closest_node_to_goal_x, closest_node_to_goal_y = goal_node_found(goal_found, visited_queue, initial_node_coord[0], initial_node_coord[1], 0, node_idx, curr_parent_idx, curr_node_coord, th_initial)
        print("abs(th_initial - th_goal): ", abs(th_initial - th_goal))
        print()
    ...
    while (len(open_dict) != 0): # Stop search when node queue is empty 
        ...
        while i < 6: # Iterate for 8 times -> 5 moves
            ...
            if valid_move == True: # Child node is valid but has not been explored yet
                ...
                # manhattan_dist = abs(child_node_x_valid - goal_node_coord[0]) + abs(child_node_y_valid - goal_node_coord[1])
                # if (manhattan_dist <= euclid_dist_threshold) and (abs(child_theta - th_goal) <= theta_threshold): # Child node equals goal node

                    visited_queue, goal_found, closest_node_to_goal_x, closest_node_to_goal_y = goal_node_found(goal_found, visited_queue, child_node_x_valid, child_node_y_valid, total_cost, node_idx, curr_parent_idx, curr_node_coord, child_theta)
                    print("Goal Node: ", visited_queue[(closest_node_to_goal_x, closest_node_to_goal_y)])
                    print()
                    print("Value -> abs(child_theta - th_goal): ", abs(child_theta - th_goal))
                    print()
